...
const renderPdf = function (req, res, next) {
    ...
    browserHelper.makePdf(postParam).then(async function (info) {
        ...
        const fileBuffer = info.fileBuffer
        const pdfBase = fileBuffer.toString("base64");
        info.fileBuffer = pdfBase
        const fileName = postParam.fileName || ("未命名" + Date.now() + ".pdf")

        let data = await processPdfMeta(pathInfo, metaInfo, ignoreMeta);

        const dbResult = await new Promise(async (resolve, reject) => {
        ...
        data.fileId = dbResult.insertedId
